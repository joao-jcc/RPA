...
import type { SearchSession, Benefit } from '../types'
            ...
            <table className="w-full text-xs">
              <thead>
                <tr className="border-b border-white/[0.06] bg-navy-900/50">
                  {headers.map((h) => (
                    <th key={h} className="px-3 py-2.5 text-left font-medium text-white/35 whitespace-nowrap">
                      {h}
                    </th>
                  ))}
                </tr>
              ...
              <tbody>
                {pageRows.map((row, i) => (
                  <tr key={i} className="border-b border-white/[0.03] hover:bg-white/[0.02]">
                    {headers.map((h) => (
                      <td key={h} className="px-3 py-2.5 text-white/55 whitespace-nowrap font-mono">
                        {row[h] ?? '—'}
                      </td>
                    ))}
                  </tr>
...
function SessionRow({ session }: { session: SearchSession }) {
  ...
  return (
    <>
      ...
      >
        ...
        <td className="px-4 py-3">
          <span className="text-xs font-mono text-white/50">{totalBenefits}</span>
        </td>
        <td className="px-4 py-3">
        ...
        <td className="px-4 py-3">
          <div className="flex items-center gap-2">
            <span className="text-[11px] text-white/30 font-mono">
              {session.finished_at?.toLocaleDateString('pt-BR') ?? '—'}
            </span>
            {persona.links?.profile && (
              <a
                href={persona.links.profile}
                target="_blank"
                rel="noopener noreferrer"
...
export function DataExplorer() {
  const { state } = useApp()
  const completed = state.sessions.filter((s) => s.status === 'done' && s.result)
      ...
      {completed.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-24 gap-4 opacity-40">
          ...
          <div className="text-center space-y-1">
            ...
            <p className="text-xs text-white/30">Complete uma busca na página Home</p>
          </div>
        </div>
      ) : (
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-white/[0.06]">
                  {['Nome', 'CPF', 'Localidade', 'Benefícios', 'Total recebido', 'Data'].map((h) => (
                    <th key={h} className="px-4 py-3 text-left text-[11px] font-medium text-white/30 uppercase tracking-wider whitespace-nowrap">
                      {h}
                    </th>
                  ))}
                </tr>
              ...
              <tbody>
                {completed.map((session) => (
                  <SessionRow key={session.job_id} session={session} />
                ))}
              </tbody>
            </table>
          </div>
